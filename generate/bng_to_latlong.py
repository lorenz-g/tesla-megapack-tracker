...
def OSGB36toWGS84(E, N):
    ...
    while N - N0 - M >= 0.00001:  # Accurate to 0.01mm
        lat = (N - N0 - M) / (a * F0) + lat
        M1 = (1 + n + (5.0 / 4) * n**2 + (5.0 / 4) * n**3) * (lat - lat0)
        M2 = (
            (3 * n + 3 * n**2 + (21.0 / 8) * n**3)
            * sin(lat - lat0)
            * cos(lat + lat0)
        )
        M3 = (
            ((15.0 / 8) * n**2 + (15.0 / 8) * n**3)
            * sin(2 * (lat - lat0))
            * cos(2 * (lat + lat0))
        )
        M4 = (35.0 / 24) * n**3 * sin(3 * (lat - lat0)) * cos(3 * (lat + lat0))
        # meridional arc
        M = b * F0 * (M1 - M2 + M3 - M4)
    ...
    lat_1 = lat - VII * dE**2 + VIII * dE**4.0 - IX * dE**6.0
    lon_1 = lon0 + X * dE - XI * dE**3 + XII * dE**5.0 - XIIA * dE**7.0
